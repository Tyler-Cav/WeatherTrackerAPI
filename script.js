...
selectedCityTitle.textContent = dayjs().format("M/D/YYYY");
  ...
  fetch(currentWeather)
    ...
    .then(function (data) {
      ...
      currentTempEL.textContent =
        "Current Temp: " + Math.round(data.main.temp) + "ºF";
        ...
        .then(function (data) {
          ...
          for (let i = 0; i < 5; i++) {
            ...
            nextDayTemp.textContent =
              "Est. Temp: " +
              Math.round(data.list[countArray[i]].main.temp) +
              "ºF";
            nextDayWind.textContent =
              "Est. Wind: " + data.list[countArray[i]].wind.speed + "MPH";
            nextDayHumidity.textContent =
              "Est. Humidity: " + data.list[countArray[i]].main.humidity + "%";
            weatherIcon.classList.add("bg-primary", "border", "rounded");
            weatherIcon.src = `https://openweathermap.org/img/w/${
              data.list[countArray[i]].weather[0].icon
            }.png`;
          }
        });
    });
...
//selecting the form
document
  .querySelector("#submitCityQuery")
  .addEventListener("submit", handleSubmitAction);
